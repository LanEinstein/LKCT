...
from distillation_engine import train_one_epoch, evaluate
...
#from LKCT import *
from DKD import *
# from shufflenet import shufflenet_v2_x2_0 as create_student
from model_v3 import mobilenet_v3_large as create_student
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./mobiele_o4',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./mobile_04',
...
def main(args):
    ...
    if args.model == 'RepLKNet-31B':
        teacher_model = build_model('litv2')
        teacher_weight_path = './lit09.pth'
        weights_dict = torch.load(teacher_weight_path, map_location=device)
        print(teacher_model.load_state_dict(weights_dict['model'], strict=False))
        student_model = create_student(num_classes=10575)
        model = DKD(student_model, teacher_model, args)
    else:
        ...
        if data_loader_val is not None:
            ...
            if max_accuracy < test_stats["acc1"]:
                ...
                if args.output_dir and args.save_ckpt:
                    ...
                    utils.save_model(
                        args=args, model=student_model, model_without_ddp=model, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch="student_best", model_ema=model_ema)
